...
class ArmXComponent(BaseExampleComponent):
    """Arm Component Base"""

    def __init__(self, name='arm', parent=None, data=None):
        super(ArmXComponent, self).__init__(name, parent)
        ...
        self.bicepRollOutputTgt = self.createOutput('bicepRoll', dataType='Xfo', parent=self.outputHrcGrp).getTarget()
        self.forearmRollOutputTgt = self.createOutput('forearmRoll', dataType='Xfo', parent=self.outputHrcGrp).getTarget()
...
class ArmXComponentGuide(ArmXComponent):
    ...
    def saveData(self):
        ...
        data = super(ArmXComponentGuide, self).saveData()
    ...
    def loadData(self, data):
        ...
        super(ArmXComponentGuide, self).loadData( data )
    ...
    def getRigBuildData(self):
        ...
        data = super(ArmXComponentGuide, self).getRigBuildData()
    ...
    def getRigComponentClass(cls):
        ...
        return ArmXComponentRig


class ArmXComponentRig(ArmXComponent):
    """Arm Component Rig"""

    def __init__(self, name='arm', parent=None):

        Profiler.getInstance().push("Construct Arm Rig Component:" + name)
        super(ArmXComponentRig, self).__init__(name, parent)
        ...
        bicepRollDef = Joint('bicepRoll', parent=defCmpGrp)
        bicepRollDef.setComponent(self)

        forearmRollDef = Joint('forearmRoll', parent=defCmpGrp)
        forearmRollDef.setComponent(self)
        ...
        # ===============  
        # Add Splice Op
        self.canvasOp = CanvasOperator('armOp', 'Kraken.Solvers.Arm')
        self.addOperator(self.canvasOp)

        # Add Att Inputs
        self.canvasOp.setInput('drawDebug', self.drawDebugInputAttr)
        self.canvasOp.setInput('rigScale', self.rigScaleInputAttr)

        self.canvasOp.setInput('bone0Len', self.armBone0LenInputAttr)
        self.canvasOp.setInput('bone1Len', self.armBone1LenInputAttr)
        self.canvasOp.setInput('ikblend', armIKBlendInputAttr)
        self.canvasOp.setInput('softIK', armSoftIKInputAttr)
        self.canvasOp.setInput('softDist', armSoftDistInputAttr)
        self.canvasOp.setInput('stretch', armStretchInputAttr)
        self.canvasOp.setInput('stretchBlend', armStretchBlendInputAttr)
        self.canvasOp.setInput('rightSide', self.rightSideInputAttr)

        # Add Xfo Inputs
        self.canvasOp.setInput('root', self.clavicleEndInputTgt)
        self.canvasOp.setInput('bone0FK', self.bicepFKCtrl)
        self.canvasOp.setInput('bone1FK', self.forearmFKCtrl)
        self.canvasOp.setInput('ikHandle', self.armIKCtrl)
        self.canvasOp.setInput('hand', self.handCtrl)
        self.canvasOp.setInput('upV', self.armUpVCtrl)

        # Add Xfo Outputs
        self.canvasOp.setOutput('bone0Out', self.bicepOutputTgt)
        self.canvasOp.setOutput('bone1Out', self.forearmOutputTgt)
        self.canvasOp.setOutput('bone2Out', self.armEndXfoOutputTgt)
        self.canvasOp.setOutput('bicepRollOut', [self.bicepRollOutputTgt])
        self.canvasOp.setOutput('forearmRollOut', [self.forearmRollOutputTgt])
        ...
        # Add Xfo Inputs
        self.outputsToDeformersKLOp.setInput('constrainers', [self.bicepOutputTgt, self.forearmOutputTgt, self.bicepRollOutputTgt, self.forearmRollOutputTgt,self.armEndXfoOutputTgt, self.handOutputTgt])

        # Add Xfo Outputs
        self.outputsToDeformersKLOp.setOutput('constrainees', [bicepDef, forearmDef, bicepRollDef, forearmRollDef, wristDef, handDef])
    ...
    def loadData(self, data=None):
        ...
        super(ArmXComponentRig, self).loadData( data )
        ...
        # Eval Operators
        self.canvasOp.evaluate()
        self.outputsToDeformersKLOp.evaluate()


from kraken.core.kraken_system import KrakenSystem
ks = KrakenSystem.getInstance()
ks.registerComponent(ArmXComponentGuide)
ks.registerComponent(ArmXComponentRig)
